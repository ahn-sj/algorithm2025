...
| 시간 복잡도 | 자료 구조         | 소요 시간  |
|---|---------------|--------|
| O(n) | PriorityQueue | 1h 10m |

큐 문제는 구현이랑 크게 다르지 않아서 그림으로 시퀀스 따라가면서 목적, 조건, 결과를 정리하는게 좋은듯

---

## 42626 더 맵게 (Not Solved)

| 시간 복잡도                               | 자료 구조 | 소요 시간 |
|--------------------------------------|-------|-------|
| 	O(log n) 삽입, O(n) 탐색 -> O(n log n) | Heap  | 

아래 세 가지 제약 사항을 봤을 때 높은 확률로 삽입과 동시에 정렬이 필요하다고 생각듦<br/>
다만 선형 리스트가 아닌 방식을 활용해야겠다는 생각을 함

> **알게된 내용**<br/>
> - Java의 우선순위 큐는 내부적으로 최소 힙(Min Heap) 구조 기반으로 동작함. 즉, 내부적으로 배열 기반의 완전 이진 트리를 사용
> - 아무 설정 없이 사용할 경우 자연 순서(Comparable)로 정렬되어 최소힙이 되는 반면, Comparator를 설정하면 최대 힙처럼 동작하게 할 수 있음 
...
- 목표: 체육수업을 들을 수 있는 학생의 최댓값
- 조건
  - 체육복은 앞뒷번호의 학생에게만 빌려줄 수 있음
  - 여벌 체육복을 가져온 학생이 체육복을 도난당했을 수 있고 이때 이 학생은 체육복을 하나만 도난당했다고 가정
